...
- Kept the spark in a fixed `56x56` pointer-free ceremony icon overlay so card text and Shop controls remain readable and immediately usable.
- Verified each mapped upgrade spent gold and changed its selected run card exactly once before spark playback, while reroll, cancel, insufficient gold, and failed image loading produced no duplicate transaction or crash.
- Verified normal playback auto-clears, reduced motion holds frame 2 before clear, forced load failure skips silently, and `390px` mobile has no horizontal overflow, text overlap, or button overlap.
- Kept walk, player defend/hurt/victory, UI icons, vocabulary card frame, and dungeon background unimplemented.
- Changed no gameplay, save, combat math, answer checking, timers, HP/shield/gold formulas, mastery, Word Energy, shop costs/values, event, elite, boss, rewards, unlocks, progression, dependencies, or deployment behavior.
