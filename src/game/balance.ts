export const MAX_WORD_MASTERY = 5;
export const MASTERY_LOW_DAMAGE_BONUS = 1;
export const MASTERY_MEDIUM_DAMAGE_BONUS = 2;
export const MASTERY_MAX_DAMAGE_BONUS = 3;

export const PLAYER_MAX_HP = 32;
export const INITIAL_SHIELD = 0;
export const STARTING_GOLD = 20;
export const GOLD_PER_MONSTER = 6;

export const SHOP_INTERVAL = 5;
export const BOSS_MONSTER_REQUIREMENT = 20;

export const UPGRADE_ATTACK_AMOUNT = 2;
export const ADD_SHIELD_AMOUNT = 3;
export const MIN_RUN_DECK_SIZE = 5;
export const MIN_DISTINCT_VISIBLE_WORDS = 4;

export const WORD_CHOICE_TIME_LIMIT = 14;
export const WORD_MATCH_TIME_LIMIT = 20;
export const WORD_SCRAMBLE_TIME_LIMIT = 22;

export const SHOP_UPGRADE_ATTACK_COST = 35;
export const SHOP_ADD_SHIELD_COST = 40;
export const SHOP_ADD_ELEMENT_COST = 45;
export const SHOP_REMOVE_CARD_COST = 30;
export const SHOP_DUPLICATE_CARD_COST = 50;

export const FIRE_BONUS_DAMAGE = 2;
export const WATER_SHIELD_GAIN = 2;
export const EARTH_ATTACK_REDUCTION = 2;
export const WIND_DEFEAT_GOLD_BONUS = 1;

export const MONSTER_ENCOUNTER_WEIGHT = 70;
export const EVENT_ENCOUNTER_WEIGHT = 20;
export const ELITE_ENCOUNTER_WEIGHT = 10;

export const ELITE_HP_MULTIPLIER = 1.5;
export const ELITE_ATTACK_BONUS = 2;
export const ELITE_GOLD_BONUS = 10;

export const TREASURE_GOLD_REWARD = 10;
export const EVENT_ATTACK_UPGRADE_AMOUNT = 1;
export const SHRINE_HEAL_AMOUNT = 5;
export const SHRINE_SHIELD_GAIN = 5;
export const ALTAR_HP_COST = 5;

export const WORD_FATIGUE_USED_THRESHOLD = 1;
export const WORD_FATIGUE_TIRED_THRESHOLD = 2;
export const WORD_FATIGUE_RESTING_THRESHOLD = 3;
export const WORD_FATIGUE_FRESH_WEIGHT = 4;
export const WORD_FATIGUE_USED_WEIGHT = 2;
export const WORD_FATIGUE_TIRED_WEIGHT = 1;
export const WORD_FATIGUE_RESTING_FALLBACK_WEIGHT = 1;
