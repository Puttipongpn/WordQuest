...
These labels describe visual-source readiness. A runtime copy is still not production-final art. Phase 71F.10 has wired six idle identities, six cast/hit sheets, five enemy attacks, five enemy defeats, four elemental effects, one shield effect, one Shop upgrade spark, three static UI icons, one decorative card frame, and one Dungeon background; the remaining copied candidate stays dormant. Phase 71G regression QA passed the wired set on desktop and `390px`, including targeted Elite Crystal Slime/Gatekeeper actions and forced fallback checks. Phase 71H passed the local production build/preview release-candidate smoke check without changing any readiness label, runtime file, source precedence, or mapping. Phase 71I promoted that exact RC to `main` and verified matching live Vercel bundle hashes; no readiness label or mapping changed.
